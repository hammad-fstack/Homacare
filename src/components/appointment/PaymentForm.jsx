...
const PaymentForm = ({ onPay }) => {
    ...
    // Card number ko 4-4-4-4 format mein dikhane ke liye
    const formatCardNumber = (value) => {
        const digits = value.replace(/\D/g, '').slice(0, 16);
        return digits.replace(/(.{4})/g, '$1 ').trim();
    };

    // Expiry ko MM/YY format mein dikhane ke liye
    ...
    const validate = () => {
        ...
        if (!/^\d{16}$/.test(rawCardNumber)) {
            newErrors.cardNumber = 'Card number should have 16 digits';
        }
        if (!cardName.trim()) {
        ...
        if (!/^\d{2}\/\d{2}$/.test(expiry)) {
            ...
            if (mm < 1 || mm > 12) {
                newErrors.expiry = 'Month should be between 01-12 ';
