...
import { LAB_STATUS_STYLES, LAB_STATUS_LABELS } from '../../config/dashboardConfig';
...
const LabTestsSection = ({ tests, emptyConfig }) => (
  <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm space-y-4 min-h-[200px]">
    <div className="flex justify-between items-center">
      ...
      <button className="text-xs text-emerald-600 font-medium">+ New Lab Test</button>
    </div>
        ...
        {tests.map((test) => (
          <div key={test.id} className="flex items-center justify-between p-4 bg-gray-50/50 rounded-xl border border-gray-100">
            <div className="flex items-center gap-3">
              <img src={test.avatar} alt={test.orderedBy} className="w-10 h-10 rounded-lg object-cover" />
              <div>
                <p className="text-xs text-gray-400">Ordered by {test.orderedBy}</p>
                <h4 className="text-sm font-semibold text-emerald-600">{test.title}</h4>
                <span className="text-[10px] text-gray-400">{test.time}</span>
              </div>
            </div>
            <button className={`text-white text-xs px-3 py-1.5 rounded-lg flex items-center gap-1 ${LAB_STATUS_STYLES[test.status]}`}>
              {LAB_STATUS_LABELS[test.status]} <ChevronRight className="w-3 h-3" />
            </button>
          </div>
        ))}
      </div>
