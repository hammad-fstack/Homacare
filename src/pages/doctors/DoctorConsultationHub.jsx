        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-4 h-[calc(100vh-140px)]">
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 overflow-y-auto">
                    ...
                    <p className="text-xs text-gray-400 mt-1 mb-4">Chat with your patient, review their history, and manage prescriptions and lab tests all in one place.</p>
                ...
                {selected ? (
                    <SimpleChatThread appointment={selected} role="doctor" otherPartyName={selected.patient_name} />
                ) : (
                            ...
                            <div className="bg-gray-50 rounded-2xl px-6 py-5 text-center space-y-3">
                                <div className="flex items-center gap-3 justify-center">
                                    <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-sm font-semibold text-gray-600">
                                        {nextUpcoming.patient_name?.[0] || '?'}
                                    </div>
                                    <div className="text-left">
                                        <p className="text-xs text-gray-400">Upcoming Consultation</p>
                                        <p className="text-sm font-bold text-gray-900">{nextUpcoming.patient_name}</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => navigate(`/doctor/consultation-hub/call/${nextUpcoming.id}`)}
...
export default DoctorConsultationHub;