        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {METRIC_CARDS.map((card) => (
              <MetricCard key={card.key} label={card.label} subtitle={card.subtitle}
                highlight={card.highlight} value={data[card.key]?.count ?? 0} />
            ))}
          </div>
          <ConsultationsSection consultation={data.upcomingConsultation} emptyConfig={EMPTY_STATE_CONFIG.consultations} />
          <LabTestsSection tests={data.labTests?.items} emptyConfig={EMPTY_STATE_CONFIG.labTests} />
        </div>
        <div className="space-y-6">
          <CalendarWidget />
